...
module.exports = function (arg, options) {
  ...
  var fn = function (config) {
    ...
    function create() {
      ...
      return heroku_client.organizations().apps().create(_.pick(config, update_params));
    }
  ...
  fn.export = function (app_name) {
    var app = heroku_client.apps(app_name);
    var organizationApp = heroku_client.organizations().apps(app_name);
  ...
  fn.pipeline = function (pipelineConfig) {
    return pipelines(heroku_client)(pipelineConfig);
  };
