...
var APP_UPDATE_PARAMS = ['build_stack', 'maintenance', 'name'];
  ...
  var fn = function (config) {
    ...
    return heroku_client.apps(config.name).info().
      then(function () {
        ...
        return heroku_client.apps(config.name).update(_.pick(config, _.intersection(top_level_keys, APP_UPDATE_PARAMS))).then(function (result) {
