...
module.exports = function (arg, options) {
  ...
  var fn = function (config) {
    ...
    function update() {
      ...
      var update_params = _.pick(config, _.intersection(top_level_keys, ALLOWED_APP_UPDATE_PARAMS));
      if(config.stack) {
  ...
  fn.export = function (app_name) {
    var app = heroku_client.apps(app_name);
    var appInfoExport = appInfo(app).export();
    var configVarsExport = configVars(app).export();
    var addonsExport = addons(app).export();
    var collaboratorsExport = collaborators(app).export();
    var featuresExport = features(app).export();
    var formationExport = formation(app).export();
    var logDrainsExport = logDrains(app).export();
    var domainsExport = domains(app).export();

    return Promise.all([appInfoExport, configVarsExport, addonsExport, collaboratorsExport, featuresExport, formationExport, logDrainsExport, domainsExport]).then(function (results) {
