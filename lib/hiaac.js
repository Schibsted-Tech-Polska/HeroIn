...
module.exports = function (arg) {
  ...
  var fn = function (config) {
    ...
    function configureApp(result) {
      ...
      var updateLogDrains = Promise.all((config.log_drains || []).map(function (url) {
        return app.logDrains().create({url: url});
      }));

      return Promise.all(updateConfigVars.concat(createOrUpdateAddons).concat(updateCollaborators).concat(updateAppFeatures).concat(updateFormation).concat(updateLogDrains));
    }
  ...
  fn.export = function (app_name) {
    ...
    var addons = app.addons().listByApp().then(function (addons) {
