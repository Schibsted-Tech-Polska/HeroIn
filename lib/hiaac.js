...
module.exports = function (arg) {
  ...
  var fn = function (config) {
    ...
    function configureApp(result) {
      ...
      var appFeatures = _.pairs(config.features).map(function(feature) {
        var updateFeature = app.features(feature[0]).update(feature[1]);
      });
      var updateAppFeatures = Promise.all(appFeatures);


      return Promise.all(updateConfigVars.concat(createOrUpdateAddons).concat(updateCollaborators).concat(updateAppFeatures));
    }
  ...
  fn.export = function (app_name) {
    ...
    return Promise.all([app_info, config_vars, addons, collaborators, features]).then(function (results) {
