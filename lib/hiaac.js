...
module.exports = function (arg) {
  ...
  var fn = function (config) {
    ...
    function configureEnvVarsAddonsCollaborators(result) {
      ...
      var existingCollaborators = heroku_client.apps(config.name).collaborators().list();
      var updateCollaborators = existingCollaborators.then(function(collaborators) {
        ...
        var createCollaborators = createEmails.map(function(email) {
          return heroku_client.apps(config.name).collaborators().create({silent: true, user: email});
        });
        var deleteCollaborators = deleteEmails.map(function(email) {
          return heroku_client.apps(config.name).collaborators(email).delete();
        });
  ...
  fn.export = function (app_name) {
    ...
    var collaborators = heroku_client.apps(app_name).collaborators().list();
