...
module.exports = function (arg, options) {
  ...
  var fn = function (config) {
    ...
    function configureApp(result) {
      ...
      var updateFormation = formation(app, app_options).configure(config.formation).then(updateAppFeatures);

      var updateLogDrains = logDrains(app).configure(config.log_drains);

      var updateDomains = domains(app).configure(config.domains);

      return Promise.all(updateConfigVars.concat(configureAddons, updateCollaborators, updateFormation, updateLogDrains, updateDomains));
    }
